...
const startgetartistPopularity = async (req, res) => {
  try {
    ...
    // Jika tidak ada filter, ambil artist dengan monthly listener dan popularity tertinggi secara global
    let artists;
    if (!region && !country) {
      // Ambil artist dengan monthly listener dan popularity tertinggi
      artists = await Artist.find({})
        .sort({ 
          monthly_listener: -1, 
          popularity: -1 
        })
        .limit(300); // Misalnya ambil top 100 artist
    } else {
      // Jika ada filter, ambil artist sesuai filter
